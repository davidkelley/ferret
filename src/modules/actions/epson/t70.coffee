...
define ['operators'], (ops) ->
  return {
    ...
    #setup alignments
    #constructs and returns an object with .left() .center()
    #and .right() accessor functions
    align: (->
    ...
    #higher 4 bits are width magnification
    #lower 4 bits are height magnification
    #ie. 0011 = 3 = 00110011 = text magnified evenly by 3
    magnify: (n) ->
      if 0 <= n <= 15
        str = Number(n).toString(2)
        #ensure concatenating 4 bits
        str = "0" + str while str.length < 4
        #join 4 bits together and determine number
      ...
      for rows in [0...options.height] by 24
        ...
        monochrome = (
          ...
          for i in [0...options.data.length] by 4
            #get the rgba values
            #byte data = [r,g,b,a,r,g,b,a,r,g,b,a,r,g,b,a,r,g,b,a....]
            r = options.data[i]
            g = options.data[i + 1]
            b = options.data[i + 2]
            a = options.data[i + 3]
            ...
            #TODO: factor out static channel threshold values
